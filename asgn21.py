...
"""
# Uncomment and adjust the following augmentation code if needed.
...
model = YOLO(r"C:\Users\arnav.shah\runs\detect\train8\weights\best.pt")
dataset_yaml = os.path.join(root_dir, 'dataset.yaml')

# Start training with checkpointing enabled (resume=True will automatically resume if a checkpoint exists)
#model.train(data=dataset_yaml, epochs=100, batch=16, workers=8, resume=True)


# ============================ 7. Model Evaluation & Video Inference ============================ #
# Evaluate the model on the validation data and visualize performance metrics
# The model.val() method returns evaluation metrics (the exact structure may vary)
metrics = model.val(data=dataset_yaml)
print("Evaluation Metrics:", metrics)

# For demonstration, assume metrics is a dictionary with keys: 'precision', 'recall', 'mAP50', 'mAP50-95'
# (Adjust extraction according to your actual output)
...
model.val(conf=True)
results = model.val()
print("Box Loss:", results.results_dict['val/box_loss'])
print("Objectness Loss:", results.results_dict['val/obj_loss'])
print("Classification Loss:", results.results_dict['val/cls_loss'])

# Test the trained model on a video
#video_path = r"C:\Users\arnav.shah\Desktop\TrafficPolice.mp4"
# The predict() method will run inference on the video and display/save the results.
